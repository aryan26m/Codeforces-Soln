...
using tople = std::tuple<int, int, int>;
void solve() {
    int n;
    cin >> n;
    vector<tople> v(n);
    rep(i, 0, n) {
        int x, y;
        cin >> x >> y;
        v[i] = make_tuple(x, y, i + 1);
    }

    sort(all(v), [](const tople& a, const tople& b) {
        if (get<0>(a) != get<0>(b)) return get<0>(a) < get<0>(b);
        return get<1>(a) < get<1>(b);
    });

    vector<tople> a1, a2;
    rep(i, 0, n / 2) a1.pb(v[i]);
    rep(i, n / 2, n) a2.pb(v[i]);

    auto cmpY = [](const tople& a, const tople& b) {
        return get<1>(a) < get<1>(b);
    };
    sort(all(a1), cmpY);
    sort(all(a2), cmpY);
    ...
    rep(i, 0, n / 2) {
        cout << get<2>(a1[i]) << " " << get<2>(a2[n / 2 - 1 - i]) << "\n";
    }
}
