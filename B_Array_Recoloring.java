...
low=mid+1;
    ...
    public static void main(String[] args) {
        ...
        Integer[] sortedA = Arrays.stream(a).boxed().toArray(Integer[]::new);
            ...
            // Simulate the painting process to find the last painted element
            int lastElement = simulatePaintingProcess(a, k);

            // Total cost is sum of first k elements + last element
            long totalCost = sumK + lastElement;

            System.out.println(totalCost);
        }
    }

    private static int simulatePaintingProcess(int[] a, int k) {
        int n = a.length;
        boolean[] isBlue = new boolean[n]; // Tracks whether an element is blue
        Queue<Integer> queue = new LinkedList<>(); // Queue for BFS

        // Select the first k largest elements and mark them as blue
        for (int i = 0; i < k; i++) {
            isBlue[i] = true;
            queue.add(i);
        }
        ...
        // Simulate the painting process using BFS
        while (!queue.isEmpty()) {
            int current = queue.poll();
            lastPainted = a[current]; // Update the last painted element

            // Check left neighbor
            if (current > 0 && !isBlue[current - 1]) {
                isBlue[current - 1] = true;
                queue.add(current - 1);
            }

            // Check right neighbor
            if (current < n - 1 && !isBlue[current + 1]) {
                isBlue[current + 1] = true;
                queue.add(current + 1);
            }
        }
