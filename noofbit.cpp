...
long long checkerbit(long long n,int x){
    long long sum=0;
    long long total=n+1;
    for(int i=x-1;i<60;i+=x){
        long long full=total/(1LL<<(i+1));
        long long left=total%(1LL<<(i+1));
        long long extra=max(left-(1LL<<i),0LL);
         sum+=full*(1LL<<i)+extra;
    }
    return sum;
}
