...
int findGreaterThanX(vector<int>& arr, int x) {
    int ans = INT_MAX;  // store best candidate
    for (int num : arr) {
        if (num > x) {
            ans = min(ans, num);
        }
    }
    return (ans == INT_MAX ? -1 : ans); // return -1 if no element > x
}
...
void solve() {
    ...
    if((temp%10)%2==1){
        ...
        if(odd){
            ...
            int x=findGreaterThanX(digit,(temp%10));
            if(x!=-1){
                cout<<2<<endl;
            }
            else{
