 ...
 * template
 */
public class N_Number_Reduction {
...
low=mid+1;
    ...
    public static void main(String[] args) {
        ...
        for(int i=1;i<=k;i++){
            if (temp.charAt(i)<min && temp.charAt(i)!='0') {
                minidx=i;
                min=temp.charAt(i);
            }
        ...
        if (minidx!=0) {
            ans.append(temp.charAt(minidx));
            temp=temp.substring(minidx+1,n);
            k=k-minidx;
        }
        else{
            temp=temp.substring(0,minidx)+temp.substring(minidx+1,n);
        }
    }      
    }
}
