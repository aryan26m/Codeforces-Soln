...
low=mid+1;
    ...
    public static void main(String[] args) {
        ...
        while(t-- >0) {
        String s=sc.next();
        if (isPerfectSquare(s)==false) {
            System.out.println(-1);
        }
        else{
            long num = Long.parseLong(s); // Convert string to long
            long sqrt = (long) Math.sqrt(num); // Get square root
            if (sqrt%2==0) {
                
                System.out.println(sqrt/2+" "+sqrt/2);
            }
            else{
                System.out.println((sqrt/2)+" "+(sqrt/2+1));
            }
        }
    }

}
 public static boolean isPerfectSquare(String numStr) {
        try {
            long num = Long.parseLong(numStr); // Convert string to long
            long sqrt = (long) Math.sqrt(num); // Get square root
