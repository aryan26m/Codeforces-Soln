...
low=mid+1;
    ...
    public static void main(String[] args) {
        ...
        while (t-- > 0) {
            ...
            long total = calculateTotal(take, n);

            while (q-- > 0) {
                int i = sc.nextInt() - 1;
                long x = sc.nextLong();
                total = updateTotal(take, total, i, x, n);
                System.out.println(total);
            }
        }
    }

    private static long calculateTotal(long[] take, int n) {
        long total = 0;
        for (int i = 0; i < n - 1; i++) {
            total += Math.min(take[i], take[i + 1]);
        }
        return total;
    }

    private static long updateTotal(long[] take, long total, int i, long x, int n) {

        if (i > 0) {
            total -= Math.min(take[i - 1], take[i]);
        }
