...
// returning those in [minVal, maxVal]. Use minVal = 1'000'000'000 for >= 10^9.
vector<long long> luckyNumbersInRange(long long minVal, long long maxVal) {
    ...
    while (!q.empty()) {
        ...
        if (x <= (LLONG_MAX - 4) / 10) q.push(x * 10 + 4);
        if (x <= (LLONG_MAX - 7) / 10) q.push(x * 10 + 7);
    }
    return result;
}
...
void solve() {
    ...
    int n;
    cin>>n;
    vi ans=luckyNumbersInRange(0,n);
    cout<<ans.size()<<endl;
}
