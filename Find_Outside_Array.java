 ...
 * template
 */
public class Find_Outside_Array {
...
low=mid+1;
    ...
    public static void main(String[] args) {
        ...
        int n = sc.nextInt();
        int a[] = enterarr(n);
       long minem=Integer.MAX_VALUE;
        long maxem=Integer.MIN_VALUE;
        for (int num : a) {
            // hs.add(num);
            minem = Math.min(minem, num);
            maxem = Math.max(maxem, num);
        }
        // if (n==1) {
    ...
    if (minem==0 && maxem==0) {
        ...
        if (minem<0) {
        System.out.println(minem+" "+minem);    
        }
        else if (maxem>0) {
            System.out.println(maxem+" "+maxem);    
        }
