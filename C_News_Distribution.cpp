...
template <class T, class U> void _print(const pair<T, U> &p) { cerr << '{'; _print(p.a); cerr << ", "; _print(p.second); cerr << '}'; }
...
template <class K, class V> void _print(const map<K, V> &m) { cerr << '{'; bool f = true; for (auto &kv : m) { if (!f) cerr << ", "; _print(kv.a); cerr << ": "; _print(kv.second); f = false; } cerr << '}'; }
...
struct UnionFind {
    ...
    UnionFind(int n) : n(n), parent(n+1), sz(n+1, 1) {
        for(int i = 1; i <= n; i++)
            parent[i] = i;
    }
