...
low=mid+1;
    ...
    public static void main(String[] args) {
        ...
        ArrayList <Pair> ar3=new ArrayList<>();
        for(int i=0;i<n;i++){
            ar1.add(new Pair(a[i],i));
            ar2.add(new Pair(b[i],i));
            ar3.add(new Pair(c[i],i));
        }
        Collections.sort(ar1, new Comparator<Pair>() {
            public int compare(Pair p1, Pair p2) {
                return Integer.compare(p1.n, p2.n);
            }
        });
        Collections.sort(ar2, new Comparator<Pair>() {
            public int compare(Pair p1, Pair p2) {
                return Integer.compare(p1.n, p2.n);
            }
        });
        Collections.sort(ar3, new Comparator<Pair>() {
            public int compare(Pair p1, Pair p2) {
                return Integer.compare(p1.n, p2.n);
            }
        });
        ...
        for(int i=n-1;i>=n-3;i--){
            for(int j=n-1;j>=n-3;j--){
                for(int k=n-1;k>=n-3;k--){
                    if(ar1.get(i).i != ar2.get(j).i && ar1.get(i).i != ar3.get(k).i && ar2.get(j).i != ar3.get(k).i){
                        int ans=ar1.get(i).n+ar2.get(j).n+ar3.get(k).n;
