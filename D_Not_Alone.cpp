    ...
    // Write your logic here
     int n;
       ...
       vi a=enterv(n);
        ...
        for (auto &x : freq) {
            int cnt = x.second / 2;
            for (int i = 0; i < cnt; i++) sides.push_back(x.first);
            if (x.second % 2 == 1) single = max(single, x.first);
        }
        if (sides.size() < 2) { 
            cout << 0 << endl;
            return;
        ...
        {
            long long sum = 0;
            for (auto x : sides) sum += 2 * x;
            long long mx = sides.back();
            if (2 * mx < sum) ans = max(ans, sum);
        }
        ...
        if (single != -1) {
            ...
            for (auto x : sides) sum += 2 * x;
            sum += single;
            long long mx = max(single, sides.back());
            if (2 * mx < sum) ans = max(ans, sum);
        }
